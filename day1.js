...
//         let curr = arr[i]
//         let digit = curr.match(findNum);
//         let currNum = '';
//         currNum += digit[0]
//         if(digit.length < 1){
//             currNum += digit[0]
//         } else{
//             currNum += digit[digit.length-1];
//         };
...
let findAll = /([0-9]|(one)|(two)|(three)|(four)|(five)|(six)|(seven)|(eight)|(nine))/g;

const findSum = (input)=>{
    let arr = input.split("\n");
    let fixArr = arr.map(e=>{
        return e.replace(/oneight/g,"oneeight").replace(/threeight/g,"threeeight").replace(/fiveight/g,"fiveeight").replace(/nineight/g,"nineeight").replace(/twone/g,"twoone").replace(/sevenine/g,"sevennine").replace(/eightwo/g,"eighttwo")
    })

    let nums = [];
    for(let i = 1;i<arr.length-1;i++){
        let curr = fixArr[i]
        let digit = curr.match(findAll);
        let currNum = '';
        let last = digit.length-1; 
        if(numbers[digit[0]]){
            currNum += numbers[digit[0]];
        } else{
            currNum += digit[0]
        }
        if(digit.length < 1){
            ...
            if(numbers[digit[last]]){
                currNum += numbers[digit[last]];
            } else{
                currNum += digit[last]
            }
        };
